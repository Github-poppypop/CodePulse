...
export const githubPlugin: FastifyPluginAsync = async (app) => {
  app.post('/webhooks/github', async (req, reply) => {
    const body = req.body as { action?: string; installation?: { id: number }; repository?: { full_name: string } };
    ...
    const repo = await prisma.repository.upsert({
      ...
      create: { owner, name, fullName: body.repository.full_name, installedById: '' },
      update: {},
    });
